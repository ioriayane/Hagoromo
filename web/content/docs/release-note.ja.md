...
- 追加
  - ポストスレッドでのインデックス表示に対応（リアルタイムフィードは除く）
- 変更
  - エラーメッセージの表示をウインドウ右下に移動（クリックすると従来どおりの表示をする）
  - リアルタイムフィードの初期化処理の見直し
...
- 追加
  - 添付画像が5枚以上の投稿に対応
- 修正
  - 引用先に5枚以上の画像が添付されているときに表示されない不具合の修正

### v0.62.0 - 2026/6/12

- 変更
  - 添付画像が5枚以上のポストに対応
  - リアルタイムフィードの編集で画像の枚数指定の候補を更新
- 修正
  - 日本語の翻訳が一部の文言で適用されなかった不具合の修正
...
- 追加
  - 認証されたときの通知に対応
- 変更
  - 投稿時の画像サイズの制限を2MByteに変更
  - Qtのバージョンを6.11.0に変更および関連する変更

### v0.60.0 - 2026/3/29

- 変更
  - リンクカードに埋め込むURLにユーザーが入力したURLをそのまま使用するように変更
  - 右下の進捗表示の改善
  - 統計とログダイアログを更新中に最小化できるように変更
- 修正
  - プロフィール画面のメニューで「メンションを送る」を選択したときにポストダイアログでハンドルが入力済みにならない不具合の修正
...
- 追加
  - キャッシュタグ（株式ティッカーシンボル）に対応
- 変更
  - ポストダイアログを表示中にその他の操作ができるように変更
  - ポストダイアログを移動できるように変更
  - ポストダイアログを複数同時に開けるように変更
- 修正
  - 投票ボタンの動作を修正
...
- 修正
  - ポストスレッドの表示で異常終了してしまう場合がある不具合の修正
...
- 更新
  - プロフィールの更新時にすぐに反映するように変更
  - プロフィール編集時にウェブサイトの内容をチェックするように変更（"https://"で始まっているか）
  - ポストなどを通報する時に選択する理由を公式に追従させる変更
