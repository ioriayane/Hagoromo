...
void RecordOperator::post()
{
    ...
    LexiconsTypeUnknown::makeFacets(
            ...
            [=](const QList<AtProtocolType::AppBskyRichtextFacet::Main> &facets) {
                ...
                connect(create_record, &ComAtprotoRepoCreateRecordEx::finished, this,
                        [=](bool success) {
                            if (success) {
                                ...
                                setProgressMessage(QString());
                                emit errorOccurred(create_record->errorCode(),
    ...
    connect(poll, &TokimekiPollOperator::finished, this,
            [=](bool success, const QString &path, TokimekiPollOperator::FunctionType type) {
                if (success && type == TokimekiPollOperator::FunctionType::GetOgp) {
                    ...
                    setProgressMessage(QString());
                    emit errorOccurred("FailGetOgpImage",
                            ...
                            const QString &via_uri)
{
    ...
    connect(create_record, &ComAtprotoRepoCreateRecordEx::finished, this, [=](bool success) {
        if (!success) {
            emit errorOccurred(create_record->errorCode(), create_record->errorMessage());
        }
        setProgressMessage(QString());
...
void RecordOperator::mute(const QString &did)
{
    ...
    connect(mute, &AppBskyGraphMuteActor::finished, this, [=](bool success) {
        if (success) {
        } else {
            emit errorOccurred(mute->errorCode(), mute->errorMessage());
        }
        setProgressMessage(QString());
        emit finished(success, QString(), QString());
        setRunning(false);
...
bool RecordOperator::bookmark(const QString &cid, const QString &uri)
{
    ...
    connect(book_mark, &AppBskyBookmarkCreateBookmark::finished, this, [=](bool success) {
        if (!success) {
            emit errorOccurred(book_mark->errorCode(), book_mark->errorMessage());
        }
        setProgressMessage(QString());
        emit finished(success, QString(), QString());
        setRunning(false);
...
void RecordOperator::deletePost(const QString &uri)
{
    ...
    connect(delete_record, &ComAtprotoRepoDeleteRecordEx::finished, this, [=](bool success) {
        if (!success) {
            emit errorOccurred(delete_record->errorCode(), delete_record->errorMessage());
        }
        setProgressMessage(QString());
        emit finished(success, QString(), QString());
        setRunning(false);
        ...
        if (success3) {
            deleteAllListItems([=](bool success2) {
                if (success2) {
                    ...
                    connect(delete_record, &ComAtprotoRepoDeleteRecordEx::finished, this,
                            [=](bool success) {
                                if (!success) {
                                    emit errorOccurred(delete_record->errorCode(),
            ...
            setProgressMessage(QString());
            emit errorOccurred(QStringLiteral("FailDeleteList"),
...
bool RecordOperator::deleteBookmark(const QString &uri)
{
    ...
    connect(book_mark, &AppBskyBookmarkDeleteBookmark::finished, this, [=](bool success) {
        if (!success) {
            emit errorOccurred(book_mark->errorCode(), book_mark->errorMessage());
        }
        setProgressMessage(QString());
                                   ...
                                   const QString &pronouns, const QString &website)
{
    ...
    connect(old_profile, &ComAtprotoRepoGetRecordEx::finished, this, [=](bool success1) {
        if (success1) {
            ...
            uploadBlob([=](bool success2) {
                if (success2) {
                    ...
                    connect(new_profile, &ComAtprotoRepoPutRecordEx::finished, this,
                            [=](bool success3) {
                                if (!success3) {
                                    emit errorOccurred(new_profile->errorCode(),
            ...
            setProgressMessage(QString());
            emit errorOccurred(old_profile->errorCode(), old_profile->errorMessage());
            emit finished(false, QString(), QString());
            setRunning(false);
        }
...
void RecordOperator::updatePostPinning(const QString &post_uri, const QString &post_cid)
{
    ...
    connect(old_profile, &ComAtprotoRepoGetRecordEx::finished, this, [=](bool success1) {
        if (success1) {
            ...
            QString old_cid = old_profile->cid();
            ComAtprotoRepoPutRecordEx *new_profile = new ComAtprotoRepoPutRecordEx(this);
            connect(new_profile, &ComAtprotoRepoPutRecordEx::finished, this, [=](bool success3) {
                if (!success3) {
                    emit errorOccurred(new_profile->errorCode(), new_profile->errorMessage());
                }
                setProgressMessage(QString());
                emit finished(success3, QString(), QString());
                setRunning(false);
                                ...
                                const QString &description, const QString &name)
{
    ...
    connect(old_list, &ComAtprotoRepoGetRecordEx::finished, this, [=](bool success1) {
        if (success1) {
            ...
            uploadBlob([=](bool success2) {
                if (success2) {
                    ...
                    connect(new_list, &ComAtprotoRepoPutRecordEx::finished, this,
                            [=](bool success3) {
                                if (!success3) {
                                    emit errorOccurred(new_list->errorCode(),
                                                       new_list->errorMessage());
                                }
                                setProgressMessage(QString());
                                emit finished(success3, QString(), QString());
                                setRunning(false);
            ...
            setProgressMessage(QString());
            emit errorOccurred(old_list->errorCode(), old_list->errorMessage());
            emit finished(false, QString(), QString());
            setRunning(false);
        }
                                      ...
                                      const QString &type, const QStringList &rules)
{
    ...
    connect(delete_record, &ComAtprotoRepoDeleteRecordEx::finished, this, [=](bool success) {
        ...
        if (type == "everybody") {
            ...
            if (!ret) {
                setProgressMessage(QString());
                emit errorOccurred("InvalidThreadGateSetting",
...
void RecordOperator::updateQuoteEnabled(const QString &uri, bool enabled)
{
    ...
    connect(record, &ComAtprotoRepoGetRecordEx::finished, this, [=](bool success) {
        ...
        connect(put, &ComAtprotoRepoPutRecordEx::finished, this, [=](bool success2) {
            ...
            setProgressMessage(QString());
            if (!success2) {
                emit errorOccurred(put->errorCode(), put->errorMessage());
            }
...
void RecordOperator::updateActivitySubscription(const QString &did, bool post, bool reply)
{
    ...
    connect(subscription, &AppBskyNotificationPutActivitySubscription::finished, this,
            [=](bool success) {
                if (success) {
                    qDebug().noquote() << "Activity subscription updated for" << did
                                       << "post:" << post << "reply:" << reply;
                } else {
                    emit errorOccurred(subscription->errorCode(), subscription->errorMessage());
                }
                emit finished(success, QString(), QString());
                setRunning(false);
...
void RecordOperator::updateNotificationDeclaration(const QString &declaration)
{
    ...
    connect(create_record, &ComAtprotoRepoPutRecordEx::finished, this, [=](bool success) {
        if (!success) {
            emit errorOccurred(create_record->errorCode(), create_record->errorMessage());
        }
        emit finished(success, QString(), QString());
...
void RecordOperator::uploadBlob(std::function<void(bool)> callback)
{
    ...
    connect(upload_blob, &ComAtprotoRepoUploadBlob::finished, this, [=](bool success) {
        if (success) {
            ...
            emit errorOccurred(upload_blob->errorCode(), upload_blob->errorMessage());
            callback(false);
        }
