    ...
    updateContentFilterLabels([=]() {
        ...
        connect(records, &ComAtprotoRepoListRecordsEx::finished, [=](bool success) {
            if (success) {
                ...
                for (const auto &record : records->recordsList()) {
                ...
                emit errorOccurred(records->errorCode(), records->errorMessage());
            }
...
void AnyFeedListModel::getPosts()
{
    ...
    connect(posts, &AppBskyFeedGetPosts::finished, [=](bool success) {
        if (success) {
            ...
            for (auto item = posts->postsList().crbegin(); item != posts->postsList().crend();
                 item++) {
